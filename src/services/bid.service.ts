...
export async function placeBid(auctionId: string, bidderId: string, amount: number) {
  ...
  const newBid = await db.transaction(async (tx) => {
    ...
    const auction = result.rows[0] as unknown as AuctionRow | undefined;

    if (!auction) throw AppError.notFound('Auction');
    if (auction.status !== 'active') throw AppError.badRequest(`Auction is not accepting bids (status: ${auction.status})`, 'AUCTION_NOT_ACTIVE');
    ...
    if (auction.seller_id === bidderId) throw AppError.badRequest('You cannot bid on your own auction', 'CANNOT_BID_OWN_AUCTION');

    const currentPrice = new Decimal(auction.current_price);
    if (bidAmount.lte(currentPrice)) throw AppError.badRequest(`Bid must exceed current price of ${currentPrice.toFixed(2)}`, 'BID_TOO_LOW');
    ...
    if (!bid) throw AppError.badRequest('Failed to place bid', 'BID_CREATION_FAILED');
