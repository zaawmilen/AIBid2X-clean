...
export async function activateAuction(auctionId: string, sellerId: string) {
  ...
  if (auction.status !== 'draft') throw AppError.conflict(`Cannot activate from status: ${auction.status}`, 'INVALID_STATUS_TRANSITION');
  if (!auction.endTime || auction.endTime <= new Date()) throw AppError.badRequest('endTime must be in the future', 'INVALID_END_TIME');
...
export async function getAuctionById(auctionId: string) {
  ...
  if (!auction) throw AppError.notFound('Auction');
  const [bidStats] = await db.select({ bidCount: count(), highestBid: sql<string>`MAX(${bids.amount})` })
