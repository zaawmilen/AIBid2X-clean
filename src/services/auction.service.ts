  ...
  try {
    await Promise.race([
      ...
      new Promise<never>((_, reject) =>
        setTimeout(() => reject(new Error(`Queue ${queueName} timed out after ${timeoutMs}ms`)), timeoutMs),
      ),
    ]);
  } catch (err) {
    // Never block the main response — just log
...
export async function createAuction(sellerId: string, input: CreateAuctionInput) {
  ...
  // Fire-and-forget — never block response on embedding generation
...
export async function activateAuction(auctionId: string, sellerId: string) {
  ...
  // Fire-and-forget — never block response on queue scheduling
