...
export async function findSimilarAuctions(
  auctionId: string,
  searchText: string,
  embedding: number[] | null,
  limit = 5,
  ...
  try {
    if (embedding && embedding.length > 0) {
      ...
      const result = await ragPool.query(
        ...
        LIMIT $4`,
        [vectorLiteral, auctionId, searchText, limit],
      );
      logger.debug({ auctionId, found: result.rows.length, mode: 'hybrid' }, 'RAG search completed');
      return result.rows as SimilarAuction[];
    } else {
      const result = await ragPool.query(
        `SELECT a.id, a.title, a.description, a.current_price, a.status,
...
export function assembleAnalysisPrompt(auction: AuctionContext, similarAuctions: SimilarAuction[]): string {
  ...
  const bidHistoryText = auction.bids.length > 0
    ? auction.bids.slice(0, 10).map(b =>
        `  • $${b.amount} — ${new Date(b.createdAt).toISOString().replace('T',' ').slice(0,19)} UTC`
      ).join('\n')
    : '  No bids placed yet';

  const marketContextText = similarAuctions.length > 0
    ? similarAuctions.map(a =>
        `  • "${a.title}" — $${a.current_price} (${a.status}) [semantic:${a.semantic_score} lexical:${a.lexical_score}]`
      ).join('\n')
