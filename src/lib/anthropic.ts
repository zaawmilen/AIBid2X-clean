...
export async function streamAnalysis(
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    stream.on('text', (textDelta: string) => { onText(textDelta); });
...
export async function streamMockAnalysis(
  currentPrice: string,
  ...
  const mockText =
    ...
    `${bidCount === 0
      ...
      : `driving the price ${priceIncreasePct}% above the $${startingPrice} starting point to $${currentPrice}.`
    } ${timeNote}\n\n` +
    `**Market Positioning**\n\n` +
    `${similarCount > 0
    ...
    `${bidCount === 0
      ...
      : `Based on current momentum, expect continued upward pressure. Auctions with ${bidCount}+ early bids typically see 15-35% additional increases before close.`
    } ${reservePrice ? `Reserve of $${reservePrice} must be met.` : 'No reserve — guaranteed sale.'}\n\n` +
    `**Key Observations**\n\n` +
