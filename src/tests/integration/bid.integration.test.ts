  ...
  it('places bid', async () => {
    await runWithCommittedFixtures(
      async (db) => {
        ...
        const auction = await createTestAuction(db, { startingPrice: 50 });
      ...
      async (tx, _withCommitted, setup) => {
        ...
        const bid = await placeBid(auction.id, bidder.id, 75);
        expect(bid).toBeDefined();
        expect(bid.amount).toBe('75.00');
        const [updated] = await tx.select().from(auctions).where(eq(auctions.id, auction.id)).limit(1);
  ...
  it('rejects lower bid', async () => {
    await runWithCommittedFixtures(
      ...
      async (tx, _withCommitted, setup) => {
        ...
        let thrown = false;
        try {
          await placeBid(auction.id, bidder.id, 50);
        } catch (e: any) {
          thrown = true;
          expect(e).toBeInstanceOf(Error);
          // AppError code is available on the instance when thrown by the service
          expect((e as any).code ?? '').toBe('BID_TOO_LOW');
        }
