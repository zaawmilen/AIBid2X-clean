...
async function fetchAuctionContext(auctionId) {
    const auctionResult = await pool.query(`SELECT id, title, description, starting_price, current_price,
            reserve_price, status, end_time, embedding
     FROM auctions WHERE id = $1`, [auctionId]);
    if (auctionResult.rows.length === 0)
        return null;
    const row = auctionResult.rows[0];
    const bidsResult = await pool.query(`SELECT b.id, b.amount, b.created_at, u.email AS bidder_email
     FROM bids b JOIN users u ON u.id = b.bidder_id
     WHERE b.auction_id = $1 ORDER BY b.created_at DESC LIMIT 20`, [auctionId]);
...
export async function streamAuctionAnalysis(auctionId, onText, onDone) {
    ...
    if (env.ANTHROPIC_API_KEY) {
        ...
        await streamMockAnalysis(auction.currentPrice, auction.startingPrice, auction.reservePrice, auction.bids.length, timeRemaining, similarAuctions.length, onText, onDone);
    }
}
